...
class DeepLGR(nn.Module):
    ...
    def forward(self, x, ext):
        ...
        if self.predictor == 'td': # tensor decomposition
            ...
            region_param = region_param.reshape(b, -1, self.n_channels, 64**2).permute(0, 3, 2, 1) # [b, H*W, n_f, 2] remark
            region_features = out.unsqueeze(3).repeat(1, 1, 1, 1) # [b, H*W, n_filters, 2]
            out = torch.sum(region_features * region_param, 2).reshape(b, 64, 64, -1)  # [b, H, W, 2]
