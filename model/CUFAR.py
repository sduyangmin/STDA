...
class CUFAR(nn.Module):
    def __init__(self, height=32, width=32, use_exf=True, scale_factor=4, 
        ...
        self.args = args
        time_span = 15 if args.dataset == 'P1' else 24
        if use_exf:
    ...
    def forward(self, x, eif):
        ...
        if self.use_exf:
            ...
            if 'XiAn' in self.args.dataset or 'ChengDu' in self.args.dataset :
                t = (eif[:, 1] * 24).long().view(-1, 1)
            else:
                t = (eif[:, 1]).long().view(-1, 1)

            if self.args.dataset == 'P1':
                t -= 7
            ...
            region_x = local_c.reshape(-1, local_c.shape[2], local_c.shape[3])
            output = self.local_sub_model(local_c)
            region_y = output.reshape(-1, output.shape[2], output.shape[3])
        ...
        for i in range(x.size(0)):
            if 'XiAn' in self.args.dataset or 'ChengDu' in self.args.dataset :
                t = int(eif[i, 1].cpu().detach().numpy()*24)
            else:
                t = int(eif[i, 1].cpu().detach().numpy())
